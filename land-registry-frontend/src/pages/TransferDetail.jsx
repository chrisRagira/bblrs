...
const STATUS_META = {
  ...
  PENDING_ADVOCATE_DOCS:        { label: "Awaiting Advocate Docs",     color: "#8b5cf6", bg: "#f5f3ff", step: 2 },
  PENDING_CLERK_VERIFICATION:   { label: "Under Clerk Review",         color: "#3b82f6", bg: "#eff6ff", step: 3 },
  PENDING_SURVEY:               { label: "Awaiting Survey",            color: "#06b6d4", bg: "#ecfeff", step: 4 },
...
const STEPS = [
  ...
  { key: "PENDING_SURVEY",             short: "Survey",    label: "Survey"              },
  ...
  return (
    <div style={{ marginBottom: 14 }}>
      ...
      <p style={{ fontSize: 14, fontWeight: 500, color: C.navy,
                  fontFamily: mono ? "monospace" : undefined, margin: 0 }}>
        {value ?? "—"}
      </p>
    </div>
  );
...
function Btn({ children, onClick, variant = "primary", disabled, loading, full, small, danger }) {
  ...
  return (
    <button onClick={!disabled && !loading ? onClick : undefined}
      style={{ ...base, ...(danger ? styles.danger : styles[variant]) }}>
      {loading ? "…" : children}
    </button>
  );
}
    ...
    <div style={{ marginBottom: 14 }}>
      {label && (
        <label style={{ display: "block", fontSize: 12, fontWeight: 600,
                        color: C.navy, marginBottom: 5, textTransform: "uppercase", letterSpacing: ".04em" }}>
  ...
  return (
    <div style={{ marginBottom: 14 }}>
      ...
      <div onClick={() => ref.current.click()} style={{
        border: `2px dashed ${file ? C.primary : "#cbd5e1"}`,
  ...
  return (
    <div style={{ overflowX: "auto", paddingBottom: 4 }}>
      <div style={{ display: "flex", alignItems: "flex-start", minWidth: 640, gap: 0 }}>
        {STEPS.map((s, i) => {
          const stepNum   = i + 1;
          const done      = terminal ? (status === "APPROVED" ? true : stepNum < current) : stepNum < current;
          const active    = !terminal && stepNum === current;
          const rejected  = terminal && status !== "APPROVED";

          const dotColor  = rejected && active ? "#ef4444"
...
function Timeline({ events }) {
  if (!events?.length) return <p style={{ fontSize: 13, color: C.textSecondary }}>No activity yet.</p>;
  return (
    <div>
      {events.map((e, i) => (
...
function BuyerActionPanel({ transfer, onSuccess }) {
  ...
  return (
    <SectionCard title="Your Decision Required" icon="🤝" accent="#f59e0b">
      <Alert type="warn">
        The seller has requested this transfer. Review the parcel and buyer details, then accept or reject.
      </Alert>
...
function AdvocateActionPanel({ transfer, onSuccess }) {
  ...
  return (
    <SectionCard title="Upload Legal Documents" icon="📋" accent="#8b5cf6">
      <Alert type="info">
        Prepare and upload the legal documents for this transfer. All documents will be hashed and stored securely.
      </Alert>
      <FileUpload label="Transfer / Sale Agreement" file={files.agreement} onChange={setFile("agreement")}
      ...
      <FileUpload label="Proof of Consideration / Payment" file={files.consideration}
        ...
        helper={transfer.transfer_type === "SALE" ? "Bank slip, M-Pesa statement, or payment proof." : "If applicable."} />
      <FileUpload label="Supporting Document" file={files.supporting} onChange={setFile("supporting")}
        helper={transfer.transfer_type === "INHERITANCE" ? "Death certificate or probate grant."
               : transfer.transfer_type === "COURT_ORDER" ? "Certified court order."
               : "Any other relevant document."} />
      <Textarea label="Notes" value={notes} onChange={setNotes} placeholder="Any notes for the registry clerk…" />
      {error && <Alert type="danger">{error}</Alert>}
...
function ClerkActionPanel({ transfer, onSuccess }) {
  ...
  const submit = async () => {
    ...
    if (decision === "REQUIRE_SURVEY" && !surveyorId.trim()) {
      setError("Surveyor National ID is required."); return;
    }
    setError(""); setLoading(true);
    try {
      await transfersApi.clerkVerify(transfer.transfer_id, {
        decision, notes, surveyorNationalId: surveyorId || undefined,
      });
      onSuccess();
    } catch (e) {
      setError(e.response?.data?.message || "Action failed.");
  ...
  return (
    <SectionCard title="Document Verification" icon="🔍" accent="#3b82f6">
      <Alert type="info">
        Review the uploaded documents and verify their authenticity before forwarding the transfer.
      </Alert>
      <div style={{ marginBottom: 14 }}>
        <p style={{ fontSize: 12, fontWeight: 600, color: C.navy, marginBottom: 8,
                    textTransform: "uppercase", letterSpacing: ".04em" }}>Decision</p>
        {[
          { value: "APPROVE",        label: "✓ Approve — documents valid",  color: "#059669" },
          { value: "REQUIRE_SURVEY", label: "📐 Require Survey",             color: "#0284c7" },
          { value: "REJECT",         label: "✕ Reject — documents invalid", color: "#dc2626" },
        ].map((opt) => (
          <label key={opt.value} style={{
            ...
            <span style={{ fontSize: 13, fontWeight: 500, color: decision === opt.value ? opt.color : C.navy }}>
              {opt.label}
            </span>
          </label>
        ))}
      </div>
      {decision === "REQUIRE_SURVEY" && (
        <Input label="Surveyor's National ID" value={surveyorId} onChange={setSurveyorId}
          placeholder="Surveyor national ID" required />
      )}
      <Textarea label="Notes" value={notes} onChange={setNotes}
        placeholder="Remarks for the audit trail…" />
      {error && <Alert type="danger">{error}</Alert>}
      <Btn full onClick={submit} loading={loading}>Submit Decision</Btn>
    </SectionCard>
  );
}
...
function SurveyorActionPanel({ transfer, onSuccess }) {
  ...
  const submit = async () => {
    ...
    if (coordsRaw.trim()) {
      try { coordinateAdjustments = JSON.parse(coordsRaw); }
      catch { setCoordsError("Invalid JSON — must be a valid coordinate object."); return; }
    }
    setError(""); setCoordsError(""); setLoading(true);
    try {
      await transfersApi.survey(transfer.transfer_id, { notes, coordinateAdjustments });
      onSuccess();
    } catch (e) {
  ...
  return (
    <SectionCard title="Survey Report" icon="📐" accent="#06b6d4">
      <Alert type="info">
        Submit your survey findings. If the parcel boundaries need updating, paste the new coordinate data below.
      </Alert>
      <div style={{ marginBottom: 14 }}>
        <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: C.navy,
          ...
          Coordinate Adjustments <span style={{ fontWeight: 400, color: C.textSecondary }}>(optional JSON)</span>
        </label>
        ...
        {coordsError && <p style={{ fontSize: 11, color: "#ef4444", marginTop: 3 }}>{coordsError}</p>}
      </div>
      <Textarea label="Survey Notes" value={notes} onChange={setNotes}
        placeholder="Describe findings, methodology, and any boundary adjustments made…" rows={4} />
      {error && <Alert type="danger">{error}</Alert>}
      <Btn full onClick={submit} loading={loading}>Submit Survey Report</Btn>
    </SectionCard>
  );
}
...
function LcbActionPanel({ transfer, onSuccess }) {
  ...
  return (
    <SectionCard title="Land Control Board Decision" icon="🏛️" accent="#10b981">
      <Alert type="warn">
        This is agricultural land. The Land Control Board must approve the transfer before it proceeds.
      </Alert>
...
function ValuerActionPanel({ transfer, onSuccess }) {
  ...
  return (
    <SectionCard title="Land Valuation" icon="📊" accent="#6366f1">
      <Alert type="info">
        Enter the government assessed value. Stamp duty will be calculated automatically
        ({rate * 100}% — {URBAN.includes(transfer.county) ? "urban" : "rural"} rate for {transfer.county}).
      </Alert>
  ...
  return (
    <SectionCard title="Stamp Duty Payment" icon="🧾" accent="#ec4899">
      {transfer.stamp_duty_kes && (
        <div style={{
          background: "#fdf2f8", border: "1px solid #fbcfe8", borderRadius: 8,
      ...
      <Alert type="info">
        Upload proof of stamp duty payment (KRA payment receipt or bank slip).
      </Alert>
    ...
    <SectionCard title="Final Registrar Approval" icon="⚖️" accent="#0ea5e9">
      ...
      {!confirm ? (
        <>
          ...
          {error && <Alert type="danger">{error}</Alert>}
          <div style={{ display: "flex", gap: 10 }}>
            <Btn full variant="success" onClick={() => setConfirm(true)}>
              ✓ Approve & Issue Title
            </Btn>
            <Btn full danger onClick={() => setConfirm("reject")}>
              ✕ Reject
            </Btn>
          </div>
        </>
        ...
        <>
          <Alert type="danger">This will permanently reject the transfer and notify the seller.</Alert>
        ...
        <>
          ...
          {error && <Alert type="danger">{error}</Alert>}
          <div style={{ display: "flex", gap: 10 }}>
            <Btn full variant="success" onClick={approve} loading={loading}>
              ✓ Confirm Approval
            </Btn>
...
export default function TransferDetail() {
  ...
  const isSeller = user?.id === transfer.previous_owner_id;
  ...
  return (
    <div className="page-wrapper--narrow" style={{ paddingBottom: 40 }}>
      ...
      <div style={{ marginBottom: 24 }}>
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between",
          ...
          <div style={{
            ...
            <span style={{ width: 7, height: 7, borderRadius: "50%", background: meta.color || "#94a3b8" }} />
      ...
      {/* ── Role-specific action panel (top) ── */}
      {isBuyer  && <BuyerActionPanel    transfer={transfer} onSuccess={load} />}
      
      {role === "ADVOCATE"       && transfer.advocate_id === user?.id && (
        <AdvocateActionPanel   transfer={transfer} onSuccess={load} />
      )}
      {role === "CLERK"          && <ClerkActionPanel      transfer={transfer} onSuccess={load} />}
      {role === "SURVEYOR"       && transfer.surveyor_id === user?.id && (
        <SurveyorActionPanel   transfer={transfer} onSuccess={load} />
      )}
      {role === "LAND_CONTROL_BOARD"    && <LcbActionPanel        transfer={transfer} onSuccess={load} />}
      {role === "COUNTY_OFFICER" && <CountyActionPanel     transfer={transfer} onSuccess={load} />}
      {role === "VALUER"         && <ValuerActionPanel     transfer={transfer} onSuccess={load} />}
      {isBuyer  && <StampDutyActionPanel  transfer={transfer} onSuccess={load} />}
      {role === "REGISTRAR"      && <RegistrarActionPanel  transfer={transfer} onSuccess={load} />}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        ...
        <SectionCard title="Transfer" icon="📝">
          ...
          <Field label="Sale Price"
            value={transfer.sale_price
              ? `KES ${Number(transfer.sale_price).toLocaleString()}`
              : "N/A"} />
          <Field label="Initiated"
            value={new Date(transfer.transferred_at).toLocaleDateString()} />
        </SectionCard>
